...
def convert_a_plus_vokipedia(
    input_file: Path, output_file: Path, band: int, klasse: int
):
    ...
    for unite in sorted(by_unite.keys(), key=lambda x: (isinstance(x, str), x)):
        ...
        bloecke = []
        for volet in sorted(
            by_volet.keys(), key=lambda x: (x is None, str(x) if x else "")
        ):
            ...
            if karten:
                bloecke.append(
                    {"name": volet_name, "bidirektional": True, "karten": karten}
                )

        if bloecke:
            inhalt.append({"name": unite_name, "bloecke": bloecke})
    ...
    result = {
        ...
        "felder": {"vorderseite": "Français", "rueckseite": "Deutsch"},
        "inhalt": inhalt,
    }
    ...
    print(
        f"Converted {input_file.name}: {len(inhalt)} units, {total_cards} cards -> {output_file}"
    )
    ...
    if "vokabeln" in data:
        ...
        for chapter in sorted(by_chapter.keys()):
            ...
            for v in by_chapter[chapter]:
                karte = {"vorne": v.get("englisch", ""), "hinten": v.get("deutsch", "")}
                if v.get("page"):
            ...
            if karten:
                bloecke.append(
                    {"name": f"Part {chapter}", "bidirektional": True, "karten": karten}
                )
        ...
        if bloecke:
            result = {
                ...
                "felder": {"vorderseite": "English", "rueckseite": "Deutsch"},
                "inhalt": [{"name": f"Unit {meta.get('unit', 4)}", "bloecke": bloecke}],
            }
...
def main():
    ...
    if input_file.exists():
        convert_emotions(input_file, output_dir / "englisch" / "emotions_klasse7.json")
